...
void World::LoadConfigSettings(bool reload)
{
	...
	// AutoBroadcast
    m_configs[CONFIG_BGBROADCAST] = sConfig.GetBoolDefault("BGBroadcast.On", false);
    m_configs[CONFIG_BGBROADCAST_CENTER] = sConfig.GetIntDefault("BGoBroadcast.Center", 0);
    m_configs[CONFIG_BGBROADCAST_CENTER] = sConfig.GetIntDefault("BGoBroadcast.Timer", 0);
...
void World::SetInitialWorldSettings()
{
	...
	sLog.outString("Loading BGbroadcasts...");
    LoadBGbroadcasts();
	...
	m_timers[WUPDATE_BGBROADCAST].SetInterval(getConfig(CONFIG_BGBROADCAST_INTERVAL));
...
void World::LoadBGbroadcasts()
{
    m_Autobroadcasts.clear();

    QueryResult_AutoPtr result = WorldDatabase.Query("SELECT message FROM bgbroadcast WHERE enabled = 1");
    ...
    if (!result)
    {
        ...
        bar.step();
        sLog.outString();
        sLog.outString(">> Loaded 0 bgbroadcasts definitions");
        return;
    }
    ...
    do
    {
        ...
        Field *fields = result->Fetch();
        std::string message = fields[0].GetCppString();
        m_BGbroadcasts.push_back(message);
        count++;
    } while (result->NextRow());

    sLog.outString();
    sLog.outString(">> Loaded %u bgbroadcast definitions", count);
}
...
void World::Update(time_t diff)
{
    ...
    if (sWorld.getConfig(CONFIG_BGBROADCAST))
    {
        if (m_timers[WUPDATE_BGBROADCAST].Passed())
        {
            m_timers[WUPDATE_BGBROADCAST].Reset();
            SendBGBroadcast();
        }
    }
...
void World::SendBGBroadcast()
{
    ...
    std::list<std::string>::const_iterator itr = m_BGbroadcasts.begin();
    std::advance(itr, rand() % m_BGbroadcasts.size());
    msg = *itr;

    uint32 abcenter = sWorld.getConfig(CONFIG_BGBROADCAST_CENTER);
    if (abcenter == 0)
    {
        sWorld.SendWorldText(LANG_BG_BROADCAST, msg.c_str());
    }
    if (abcenter == 1)
    {
        WorldPacket data(SMSG_NOTIFICATION, (msg.size()+1));
