    ...
    if (uVictim && uVictim->GetTypeId() == TYPEID_PLAYER && GetZoneId() == 2037)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if(AddItem(itemId, count))
                ChatHandler(this).SendSysMessage(BG_AWARD_POINT);
        }
    }
...
void Player::UpdateZone(uint32 newZone)
{
    ...
    if (m_zoneUpdateId != 2037 && newZone == 2037 && !HasItemCount( 999951, 1, false ))
    {
      ...
      ChatHandler(this).PSendSysMessage("You cannot enter Quel Thalas Keep without Keep Token. You must join to battle with Battlemaster!");
    }  
