...
void BattleGround::HandleKillPlayer(Player *player, Player *killer)
{
        ...
        for (BattleGroundScoreMap::const_iterator itr = m_PlayerScores.begin(); itr != m_PlayerScores.end(); ++itr)
         {
            if (itr->second->HonorableKills != 0)
                break;
            if ((*itr) == (*m_PlayerScores.rbegin()))
                PSendMessageToAll(LANG_UTA_FIRST_BLOOD, CHAT_MSG_BG_SYSTEM_NEUTRAL, NULL, killer->GetName());
        }
    ...
    if (m_Players[player->GetGUID()].Public >= IntervBetKills)
        PSendMessageToAll(LANG_UTA_INTERRUPTED_SERIES, CHAT_MSG_BG_SYSTEM_NEUTRAL, NULL, killer->GetName(), player->GetName());
    m_Players[player->GetGUID()].Private = 0;
    m_Players[player->GetGUID()].Public = 0;
    ++m_Players[killer->GetGUID()].Private;
    ++m_Players[killer->GetGUID()].Public;
    // Public award
    uint16 divided = m_Players[killer->GetGUID()].Public / IntervBetKills;
    if (divided > 0 && (m_Players[killer->GetGUID()].Public % IntervBetKills) == 0)
    {
        ...
        PSendMessageToAll(publicAwardEntry, CHAT_MSG_BG_SYSTEM_NEUTRAL, NULL, killer->GetName());
    }

    m_Players[killer->GetGUID()].LastKillTime = getMSTime();

    // Private award
    const uint16 maxTimeIntervBetwKills = 5000;
    if (m_Players[killer->GetGUID()].Private >= 2)
    {
        if (getMSTimeDiff(getMSTime(),m_Players[killer->GetGUID()].LastKillTime) <= maxTimeIntervBetwKills)
        {
            int32 privateAwardEntry = LANG_UTA_DOUBLE_KILL + (m_Players[killer->GetGUID()].Private - 2);
            if (privateAwardEntry > LANG_UTA_HOLY_SHIT)
                privateAwardEntry = LANG_UTA_HOLY_SHIT;

            killer->GetSession()->SendNotification("%s", killer->GetSession()->GetOregonString(privateAwardEntry));
        }
        else
            m_Players[killer->GetGUID()].Private = 1;
    }
