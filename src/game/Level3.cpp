...
bool ChatHandler::HandleReloadAllCommand(const char*)
{
	...
	HandleReloadBGbroadcastCommand("");
    return true;
}
...
bool ChatHandler::HandleReloadBGbroadcastCommand(const char*)
{
    sLog.outString("Re-Loading BGbroadcast...");
    sWorld.LoadBGbroadcasts();
    SendGlobalGMSysMessage("DB table bgbroadcast reloaded.");
    return true;
}
...
bool ChatHandler::HandleDevAnnounceCommand(const char* args)
{
    ...
    sWorld.SendWorldText(112235, m_session->GetPlayerName(), args);
...
bool ChatHandler::HandleAdminAnnounceCommand(const char* args)
{
  ...
  sWorld.SendWorldText(112236, m_session->GetPlayerName(), args);

    return true;
}

bool ChatHandler::HandleHeadGMAnnounceCommand(const char* args)
{
	if (!*args)
		return false;

	sWorld.SendWorldText(112242, m_session->GetPlayerName(), args);
