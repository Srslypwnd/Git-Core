...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
    ...
    if (m_Players[key].GetTotalReports() > sWorld.getConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
    {
		if (sWorld.getConfig(CONFIG_ANTICHEAT_KICK_ENABLE))
		{
			// display warning at the center of the screen, hacky way?
			std::string str = "";
			str = "|cFFFFFC00[Anticheat]|cFF00FFFF[|cFF60FF00 " + std::string(player->GetName()) + " |cFF00FFFF] Found cheater! Kicked or Banned";
			WorldPacket data(SMSG_NOTIFICATION, (str.size()+1));
			data << str;
			sWorld.SendGlobalGMMessage(&data);
		...
		else
		{
			// display warning at the center of the screen, hacky way?
			std::string str = "";
			str = "|cFFFFFC00[Anticheat]|cFF00FFFF[|cFF60FF00 " + std::string(player->GetName()) + " |cFF00FFFF] Found cheater!";
			WorldPacket data(SMSG_NOTIFICATION, (str.size()+1));
			data << str;
			sWorld.SendGlobalGMMessage(&data);
		}
