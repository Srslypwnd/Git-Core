...
bool ChatHandler::HandleChatAnnounceCommand(const char* args)
{
    if(!*args)
        return false;

    sWorld.SendWorldText(112237, m_session->GetPlayerName(), args);

    return true;
}

bool ChatHandler::HandleEventAnnounceCommand(const char* args)
{
    if(!*args)
        return false;

    sWorld.SendWorldText(112233, m_session->GetPlayerName(), args);

    return true;
}

bool ChatHandler::HandleHeadEVMAnnounceCommand(const char* args)
{
	if (!*args)
		return false;

	sWorld.SendWorldText(112241, m_session->GetPlayerName(), args);

	return true;
}

bool ChatHandler::HandleVIPAnnounceCommand(const char* args)
{
	if (!*args)
		return false;

	sWorld.SendWorldText(112240, m_session->GetPlayerName(), args);
...
bool ChatHandler::HandleNameAnnounceCommand(const char* args)
{
    ...
    sWorld.SendWorldText(112234, m_session->GetPlayerName(), args);
...
bool ChatHandler::HandleVipBuffCommand(const char* /*args*/)
{
    ...
    chr->Unmount();
    chr->RemoveAurasDueToSpell(SPELL_AURA_MOUNTED);
...
bool ChatHandler::HandleVipFlyCommand(const char* /*args*/)
{
    Player *chr = m_session->GetPlayer();

        // Zakaz Pouzivania v combate
    if (chr->isInCombat())
    {
        SendSysMessage("Tento command nemozes pouzit v combate!");
        SetSentErrorMessage(true);
    ...
    {
        SendSysMessage("Tento command nemozes pouzit v Instacii!");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    if (chr->isDead())
    {
        SendSysMessage("Tento command nemozes pouzit ked si mrtvy!");
        SetSentErrorMessage(true);
        return false;
    }
    ...
    chr->Unmount();
    chr->RemoveAurasDueToSpell(SPELL_AURA_MOUNTED);
    chr->RemoveAurasDueToSpell(SPELL_AURA_MOD_STEALTH);
    chr->RemoveAurasDueToSpell(SPELL_AURA_MOD_SHAPESHIFT);
    chr->RemoveAurasDueToSpell(SPELL_AURA_FEIGN_DEATH);
       
       if (chr->GetSession()->GetSecurity() == 1) //if VIP
       {
           chr->AddAura(32239, chr);  // VIP MOUNT
